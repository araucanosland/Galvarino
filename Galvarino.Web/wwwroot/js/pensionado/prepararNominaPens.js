...
const metodos = {
    ...
    render: function () {
        ...
        $.each(_ingresados, function (i, exp) {
            ...
            $.each(exp.obtenido.documentos, function (i, doc) {
                ...
                internos += `<li><a href="#">${doc.nombreDocumento} <i class="glyphicon ${calssE}" /></a></li>`
    ...
    avanzarWf: function () {
        ...
        $.ajax({
            type: "POST",
            url: `/api/pensionado/wf/v1/preparar-nomina`,
...
$(function () {
    ...
    $(document).on(eventos.pistolaDisparada, function (event, params) {
        ...
        $.ajax({
            type: "GET",
            url: `/api/pensionado/wf/v1/obtener-expediente/${expediente.folioCredito}/${etapa}`
        }).done(function (data) {
    ...
    $("#btn-exportar-excel").on("click", function () {
        ...
        if (value == "No matching records found") {
            ...
            location.href = `${BASE_URL}/api/pensionado/reportes/Preparar-Nomina/${etapa_actual}`;
        }
