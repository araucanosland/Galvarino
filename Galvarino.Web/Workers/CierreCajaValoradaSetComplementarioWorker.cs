...
{
    public class CierreCajaValoradaSetComplementarioWorker : BackgroundService
    {
        ...
        public CierreCajaValoradaSetComplementarioWorker(ILogger<CierreCajaValoradaSetComplementarioWorker> logger, IServiceProvider services, IConfiguration configuration, IHubContext<NotificacionCajaCerradaHub> notificion)
        {
            _logger = logger;
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                if (caja != null)
                {
                    ...
                    try
                    {
                        ...
                        {
                            var elExpediente = _context.ExpedientesCreditos.Include(d => d.Credito).SingleOrDefault(x => x.Credito.FolioCredito == item && x.TipoExpediente == TipoExpediente.Complementario);
                         ...
                         var expedienteComplementario = _context.ExpedientesComplementarios.Where(x => x.FolioCredito == elExpediente.Credito.FolioCredito).FirstOrDefault();
                        ...
                        await _context.SaveChangesAsync();
                        await _wfservice.AvanzarRango(ProcesoDocumentos.ETAPA_NOMINA_SET_COMPLEMENTARIO, ProcesoDocumentos.ETAPA_DESPACHO_A_CUSTODIA_SET_COMPLEMENTARIO, ticketsAvanzar, caja.Usuario);
